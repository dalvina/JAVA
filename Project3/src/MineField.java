...
public class MineField {
   ...
   public void populateMineField(int row, int col) {
	   ...
	   while(remainMines > 0){
		   ...
		   if((!mineArray[randomRow][randomCol]) && (randomRow != row) && (randomCol != col)) {
   ...
   */
   public int numAdjacentMines(int row, int col) {
      assert inRange(row, col);// DUMMY CODE so skeleton compiles
      int numAjacentMines = 0;
      for(int i = row-1; i <= row+1; i++) {
    	  for(int j = col-1; j <= col+1; j++) {
    		  if(inRange(i,j) && mineArray[i][j] && (i!=row || j!= col)) {
    			  numAjacentMines++;
    		  }
    	  }
      }
   ...
   */    
   public boolean hasMine(int row, int col) {
	   assert inRange(row,col);
       return mineArray[row][col];       // DUMMY CODE so skeleton compiles
   }
